...
fn update_particle_target_angles(
    ...
    for children in &cells {
        ...
        for (i, (entity, current_angle)) in particles.into_iter().enumerate() {
...
fn particle_movement(
    mut commands: Commands,
    query: Query<(Entity, &Transform, &TargetAngle), (With<Particle>, Changed<TargetAngle>)>,
) {
    for (entity, transform, angle) in &query {
        let distance = CELL_RADIUS - PARTICLE_RADIUS;
        let x = distance * angle.0.cos();
        let y = distance * angle.0.sin();

        /*
         let motion = EasedMotion {
                    from: *transform,
                    to: transform.with_translation(Vec3::new(x, y, transform.translation.z)),
                    timer: Timer::from_seconds(1.0, TimerMode::Once),
                    easing: EaseFunction::CubicInOut,
                };
                commands.entity(entity).insert(motion);
        info!("motion to {:?}", motion);
        */
...
fn particle_distance_movement(
    time: Res<Time>,
    mut query: Query<(&mut CurrentDistance, &TargetDistance), With<Particle>>,
) {
    for (mut current, target) in &mut query {
        let diff = target.0 - current.0;
        let step = diff * time.delta_secs() * PARTICLE_DISTANCE_SPEED;
        current.0 += step;
    }
}
